...
- [x] Explain what a token is used for.
  ...
  -- Answer: A token is an encrypted string which gets passed between the client and the server; it acts as client authentication for the backend. 


- [x] What steps can you take in your web apps to keep your data secure?

 -- Answer: Some very basic steps we can take to protect our web apps are: using protected routing, https, and correctly handling our token. 

- [x] Describe how web servers work.

 -- Answer: The term 'web server' can actually refer to one of two things: the physical computer that is hosting our backend, or the software which allows a client to communicate with the physical server. The software uses the url and domain name to locate the specific, physical server it needs, and then acts as an intermediary to pass data back and forth. 
